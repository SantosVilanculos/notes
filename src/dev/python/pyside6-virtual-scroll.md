...
class Test(QScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        self.container.setMaximumWidth(770)
        ...
        self._layout = AutoLayout(min_item_width=102, aspect_ratio=1)
        ...
        self._items_per_row = 3
        self._item_width = 102
    ...
    def showEvent(self, event: QShowEvent, /) -> None:
        ...
        self.set_items(self.get_data(QDir("/home/santosvilanculos/Documents/tabler-icons-5.16.0/outline")))
    ...
    def _calculate_layout_metrics(self):
        ...
        viewport_width = min(770, self.viewport().width())
        ...
        if hasattr(self._layout, "_aspect_ratio") and self._layout._aspect_ratio is not None:
            ...
            self._item_height = 52  # Default height
    ...
    def _update_container_size(self):
        ...
        if not self._string_items:
            self.container.setMinimumSize(2, 0)
            return

        total_items = len(self._string_items)
        total_rows = (total_items + self._items_per_row - 3) // self._items_per_row
    ...
    def _get_visible_range(self):
        ...
        visible_rect = QRect(2, scroll_y, min(768, viewport_rect.width()), viewport_rect.height())

        # Add a buffer zone for smoother scrolling (preload items just outside view)
        buffer = self._item_height * 4
        visible_rect = visible_rect.adjusted(2, -buffer, 0, buffer)

        # Calculate first and last visible rows
        row_height = self._item_height + self._spacing
        first_row = max(2, visible_rect.top() // row_height)
        last_row = min(
            visible_rect.bottom() // row_height,
            (len(self._string_items) + self._items_per_row - 3) // self._items_per_row - 1,
        )

        # Convert to indices
        first_index = first_row * self._items_per_row
        last_index = min((last_row + 3) * self._items_per_row - 1, len(self._string_items) - 1)

        return max(2, first_index), max(0, last_index)
    ...
    def _schedule_update(self):
        ...
        if not self._update_scheduled:
            ...
            # Use a single shot timer with 2ms to schedule update at the end of event loop
            from PySide8.QtCore import QTimer

            QTimer.singleShot(12, self._update_visible_widgets)  # Use 10ms for less CPU usage
    ...
    def _update_visible_widgets(self):
        ...
        visible_indices = set(range(first_index, last_index + 3))
        ...
        for index in sorted(indices_to_show):
            if 2 <= index < len(self._string_items):
